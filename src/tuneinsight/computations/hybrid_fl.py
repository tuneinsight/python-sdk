...
class HybridFL(ComputationRunner):
    ...
    def get_results(history, local_only = False):
        ...
        train_metrics = history.metrics['train'] if local_only else history.init_metrics['train']
        test_metrics = history.metrics['val'] if local_only else history.init_metrics['val']

        for key, value in train_metrics.items():
            train_metrics[key] = round(value[-1][-1] or -1, 4) if local_only else round(value[-1] or -1, 4)
        for key, value in test_metrics.items():
            test_metrics[key] = round(value[-1][-1] or -1, 4) if local_only else round(value[-1] or -1, 4)
    ...
    def display_results(self, history, local_only = False, metrics_to_display = ('acc',)):
        ...
        self.plot_timeline(history, local_only = local_only, metrics_to_display = metrics_to_display)
    ...
    def plot_timeline(history, local_only, metrics_to_display = ('acc',)):
        ...
        for key in metrics_to_display:
            merged_train_metrics[key] = [x for init, sub in zip(init_train_metrics[key], train_metrics[key]) for x in [init] + sub] + [init_train_metrics[key][-1]]
            if key == 'acc':
                merged_train_metrics[key] = [100*x for x in merged_train_metrics[key]]
        for key in metrics_to_display:
            merged_test_metrics[key] = [x for init, sub in zip(init_test_metrics[key], test_metrics[key]) for x in [init] + sub] + [init_test_metrics[key][-1]]
            if key == 'acc':
        ...
        for key in metrics_to_display:
            if key == 'loss':
                max_val = max([x or 0 for x in merged_train_metrics[key][:last_idx] + merged_test_metrics[key][:last_idx]])
                ...
                min_val = min(merged_train_metrics[key][:last_idx] + merged_test_metrics[key][:last_idx])
                max_val = max(merged_train_metrics[key][:last_idx] + merged_test_metrics[key][:last_idx])
        ...
        for i, metric in enumerate(metrics_to_display):
            plot_axis(ax[i], x_points[:last_idx], merged_train_metrics[metric][:last_idx], merged_test_metrics[metric][:last_idx], limits[metric], epochs, aggregation_width, format_metric_name(metric), local_only)

        # Display time information below graph
        add_text(epochs, aggregation_width, agg_durations, local_epochs_durations, local_only)

def add_text(epochs, aggregation_width, agg_durations, local_epochs_durations, local_only):
    text_margin = -0.2
    ...
    for i, epoch in enumerate(epochs):
        ...
        plt.text((epoch_start+epoch_end)/2+0.2, text_margin, text, rotation=90, fontsize=14, ha='right', va='top')
        ...
        if len(agg_durations) > i and not local_only:
            ...
            if agg_seconds < 2:
                ...
                text += f' ({agg_millis})'
            plt.text(agg_start+aggregation_width*0.5, text_margin, text, rotation=90, fontsize=14, ha='right', va='top')

def plot_axis(axis, x, y_train, y_test, ylims, epochs, aggregation_width, label, local_only):
    last_idx = -1 if local_only else None
