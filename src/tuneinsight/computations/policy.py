...
def display_policy(
    p: models.ComputationPolicy, detailed: bool = False, show_queries: bool = False
):
    ...
    if p.restrict_preprocessing_operations:
        display(
            Markdown(
                f"Set of restricted preprocessing operations: `{[op.value for op in p.authorized_preprocessing_operations]}`"
            )
        )
    if p.restrict_data_source_queries:
        display(
            Markdown("The policy only allows for limited queries on the data source")
        )
        if show_queries:
...
def display_threshold(text: str, t: models.Threshold):
    if t.type == models.ThresholdType.FIXED:
        ...
        display(
            Markdown(
                f"{text}: ${t.relative_factor} N$ (where $N$ is the local dataset size) "
            )
        )
...
def display_dp_policy(dp: models.DPPolicy):
    ...
    if isinstance(dp.authorized_variables, list) and len(dp.authorized_variables) > 0:
        ...
        display(
            Markdown(
                f"Set of authorized variables: `{dp.authorized_variables}` _(any non-matching variable is automatically dropped)_"
            )
        )
    ...
    if not isinstance(dp.min_global_dataset_size, Unset):
        params_set = True
        display(
            Markdown(f"minimum global dataset size: ${dp.min_global_dataset_size}$")
        )
    if not isinstance(dp.min_frequencies, Unset):
        ...
        display(
            Markdown(
                "This ensures that any Frequency $F$ is such that $F = 0$ or $F = N$ or $T <= F <= N - T$, where $N$ is the local dataset size and $T$ is the value of the threshold"
            )
        )
    ...
    if not isinstance(dp.noise_parameters, Unset):
        ...
        display(
            Markdown(
                f"epsilon: ${np.epsilon}$ (privacy budget, ideally should be set to values $< 1$ to ensure enough privacy)"
            )
        )
        display(
            Markdown(
                f"sensitivity: ${np.sensitivity}$ (maximum difference of result when computing over two neighboring datasets)"
            )
        )
        if np.discrete:
            display(
                Markdown(
                    f"delta: ${np.delta}$ (secondary noise parameters should be set to ~ $1 / N$ where $N$ is an expected dataset size)"
                )
            )
    if not params_set:
