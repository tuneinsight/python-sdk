...
class Aggregation(ComputationRunner):
    ...
    def new_model() -> models.StatisticalAggregation:
        return models.StatisticalAggregation(
    ...
    def run(
        self, comp: models.StatisticalAggregation, local: bool
    ) -> models.FloatMatrix:
    ...
    def sanitize_output(df: pd.DataFrame) -> pd.DataFrame:
        """
        sanitize_output removes any invalid values (can be due to aggregation overflows) from the output dataset

        Args:
            df (pd.DataFrame): the output dataset to sanitize

        Returns:
            pd.DataFrame: the sanitized version of the dataset
        """
    ...
    def group_by_to_dataframe(
        ...
        for cat in cat_labels:
            ...
            for col, vals in self.count_columns.items():
                ...
                tmp.append(
                    round(
                        counts[cat + col + "other"] / count * 100, self.float_precision
                    )
                )
        ...
        for i, col_info in enumerate(column_infos):
            ...
            if col_info.value_type == models.ColumnInfoValueType.ROWCOUNT:
                if col_info.origin_column == UNSET:
                    ...
                    counts[
                        cat_label + col_info.origin_column + col_info.origin_value
                    ] = int(vals[i])
            else:
    ...
    def group_by_value(self, local: bool = False) -> pd.DataFrame:
        ...
        bin_operation: models.BinningOperation = models.BinningOperation(
        ...
        # Process results
        counts, totals, categories = self.process_group_by_columns(
            result.contextual_info.columns_info,
            result.data[0],
            self.value_to_categorical_label(),
        )
        return self.group_by_to_dataframe(
    ...
    def group_by_interval(self, local: bool = False) -> pd.DataFrame:
        ...
        counts, totals, categories = self.process_group_by_columns(
            result.contextual_info.columns_info,
            result.data[0],
            self.interval_to_categorical_label(self.interval),
        )
        return self.group_by_to_dataframe(
            cat_labels=categories, counts=counts, totals=totals
        )
