...
class Statistics:
    ...
    def as_table(self) -> pd.DataFrame:
        ...
        for res in self.results:
            data.append([res.name,res.mean,res.variance,res.min_,res.median,res.max_,res.iqr])
...
class DatasetStatistics(ComputationRunner):
    ...
    def create_subgroups(self,variable_name:str,column: str,values: List[str],numerical=False):
        ...
        for v in values:
            ...
            self.new_variable(name=new_var_name,variable=variable.variable,min_bound=variable.min_bound,max_bound=variable.max_bound)
            self.set_filter(variable_name=new_var_name,column=column,comparator=models.ComparisonType.EQUAL,value=v,numerical=numerical)


    def set_filter(self,variable_name: str,column: str,comparator:models.ComparisonType, value:Any, numerical:bool = True):
        if variable_name not in self.variables:
            raise Exception(f'no such variable: {variable_name}')
        f = models.Filter(type=models.PreprocessingOperationType.FILTER, col_name=column, comparator=comparator, value=str(value), numerical=numerical)
