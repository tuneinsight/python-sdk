...
class SurvivalAggregation(ComputationRunner):
    ...
    def set_matching(self,matching_organization: str, matching_columns: List[str],fuzzy_matching: bool = False):
        ...
        for c in matching_columns:
            self.matching_columns.append(models.MatchingColumn(name=c,fuzzy=fuzzy_matching))


    def compute_survival(self,survival_parameters: SurvivalParameters) -> Dict[str,pd.DataFrame]:
        # Set parameters and run computation
        model = models.SurvivalAggregation(type=models.ComputationType.SURVIVALAGGREGATION)
        ...
        if len(fm.data) != len(self.subgroups) + 1:
            raise Exception(f"result dimensions mismatch, expected {len(self.subgroups) + 1} rows, got {len(fm.data)}")
        result_mapping = {}
