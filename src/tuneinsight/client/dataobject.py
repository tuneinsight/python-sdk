...
class DataObject:
    ...
    def load_data_from_bytes(self,data: bytes):
        definition = models.PutDataObjectDataMultipartData(File(payload=io.BytesIO(initial_bytes=data),file_name="test"))
        doResp: Response[models.DataObject] = put_data_object_data.sync_detailed(data_object_id=self.get_id(),client=self.client,multipart_data=definition)
        validate_response(doResp)
        self.model = doResp.parsed

    def get_raw_data(self) -> bytes :
        resp: Response[File] = get_data_object_raw_data.sync_detailed(data_object_id=self.get_id(),client=self.client)
