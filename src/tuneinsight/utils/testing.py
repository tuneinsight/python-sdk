...
class TestCase3Nodes(unittest.TestCase):
    ...
    def verify_success(self, response: Response):
        ...
        if (
            response.status_code < status_codes_min
            or response.status_code > status_codes_max
        ):
    ...
    def compare_csv(
        ...
        expected_vals: List[List[Any]],
        actual_vals: List[List[Any]],
    ):
    ...
    @staticmethod
    def get_csv(
        filename: str, with_header: bool = True
...
def partition_dataframe(
    df: pd.DataFrame, num: int = 3, seed: int = 0
) -> List[pd.DataFrame]:
    if num == 1:
        return [df]
    if num > len(df):
