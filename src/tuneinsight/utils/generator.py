...
class PatientGenerator(MixedGenerator):
    ...
    def random_district(self) -> str:
        return self.choices(self.district_choices, weights=self.district_weights, k=1)[
            0
        ]
    ...
    def age_to_height(self, age: int) -> float:
        return (
            (self.random() * 0.2 + 0.8 + self.random() * 0.1)
            * self.get_age_average(age)
            * self.height_factor
        )
