...
def generate_survival_curve(num_months: int,death_rate: float,shift: float) -> List[float]:
    c1 = -10 * death_rate
    c2 = shift
...
class CustomGenerator(Random):
    ...
    def generate_genome(self,num_snps: int,locus_path: str,locus_column: str,keep: List[str] = None):
        locuses = self._get_sample_locuses(num_snps,locus_path,locus_column,keep)
        for i,locus in enumerate(locuses):
            self.df[locus] =  self.choices(self.snp_choices,weights=self.snp_weights,k=self.rows)
            if i % 25 == 0:
    ...
    def add_survival_event(self,event: str,start_event: str,num_months: int =12 ,death_rate: float =0.9,shift: float =0.0,condition=None):
        if not event in self.df:
            self.df[event] = pd.NaT
        survivals = generate_survival_curve(num_months=num_months,death_rate=death_rate,shift=shift)
    ...
    def add_phenotype(self,name: str,correlated_snp_indexes: List[int],weights: List[float],covariates: List[str] = None,covariates_weights: List[float] = None):
    ...
    def apply_snp_correlation(self,num_regions: int,condition,correlation_factor: float = 0.9):
        snp_indices,probabilities = self.get_random_snp_clusters(num_clusters=num_regions,num_vals=len(self.locuses),scale=correlation_factor)
        ...
        for i,snp_index in enumerate(snp_indices):
            ...
            self.df[snp] = self.df.apply(lambda x,y=snp,z=prob: correlate(x,y,z),axis=1)


    def add_variable(self,variable_name: str,loc: float=0.0,scale:float= 1.0,normalize: bool = True):
        self.df[variable_name] = self.np_rng.normal(loc,scale,size=self.df.shape[0])
        if normalize:
            self.normalize_min_max(variable_name)


    def add_relation(self,new_variable: str,related_variable:str,loc: float=0.0,scale:float= 1.0):
    ...
    def apply_variable(self,variable_name: str,condition,loc: float=0.0,scale:float= 1.0):
        vals = self.np_rng.normal(loc,scale,size=self.df.shape[0])
        def apply_func(x):
            if condition(x):
                return vals[x.name]
    ...
    def apply_correlation(self,column: str,condition,applied_value: float=1,correlation_factor: float= 0.9):
        def correlate(x):
